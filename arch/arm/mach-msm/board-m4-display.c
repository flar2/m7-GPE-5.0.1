...
static int m4_detect_panel(const char *name)
{
#if 0
...
static struct perf_lock m4_perf_lock;

static struct dsi_buf m4_panel_tx_buf;
static struct dsi_buf m4_panel_rx_buf;
...
static char counter_measure_on[] = {0xB4, 0x12}; /* use counter measure to avoid flicker */
...
// Remove the VCOMDC3 setting to prevent flicker issue.
...
static char bl_ctl[4] = {0x53, 0x24}; /* BCTRL on, DD off, BL on */
...
static int m4_lcd_on(struct platform_device *pdev)
{
...
static int m4_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
...
static void m4_display_off(struct msm_fb_data_type *mfd)
{
	...
	time_consume = (after.tv_sec - before.tv_sec) * USEC_PER_SEC + (after.tv_usec - before.tv_usec);

	/* We shuold guarantee to delay 130ms when the host issue enter sleep command to
	   panel */
	if(time_consume < 130000)
		hr_msleep((130000-time_consume)/1000);
...
static struct msm_fb_panel_data m4_panel_data = {
	...
	.display_on	= m4_display_on,
	.display_off	= m4_display_off,
#ifdef CONFIG_MSM_CABC_VIDEO_ENHANCE
...
/* DSI_BIT_CLK at 507MHz, 4 lane, RGB888 */
	...
	/* Set DSI PLL optimal values at PLL_CTRL registers 16 and 17 to
         ...
         * Change DSI1_DSIPHY_PLL_CTRL_16 from 0x00 to 0x0e
         * Change DSI1_DSIPHY_PLL_CTRL_17 from 0x20 to 0x01
         */
	0x00, 0x14, 0x03, 0x00, 0x02, 0x0e, 0x01, 0x00, 0x01},
};

static struct mipi_dsi_reg_set dsi_sharp_cmd_mode_reg_db[] = {
	{0x0300, 0xC0},	/* DSI1_DSIPHY_LN0_CFG0 */
	{0x0304, 0xEF},	/* DSI1_DSIPHY_LN0_CFG1 */
	{0x0318, 0x00},	/* DSI1_DSIPHY_LN0_TEST_STR1 */
	...
	{0x0400, 0xC0},	/* DSI1_DSIPHY_LNCK_CFG0 */
	{0x0404, 0xEF},	/* DSI1_DSIPHY_LNCK_CFG1 */
	{0x0418, 0x88},	/* DSI1_DSIPHY_LNCK_TEST_STR1 */
};
...
static int __init m4_init_panel(void)
{
	if(panel_type == PANEL_ID_NONE) {
	...
	mipi_dsi_buf_alloc(&m4_panel_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&m4_panel_rx_buf, DSI_BUF_SIZE);
...
device_initcall_sync(m4_init_panel);
